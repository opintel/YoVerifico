package la.opi.verificacionciudadana.util;

/**
 * Created by Jhordan on 05/02/15.
 */
public class Config {

    public final static String ROBOTO_MEDIUM = "fonts/Roboto-Medium.ttf";
    public final static String ROBOTO_REGULAR = "fonts/Roboto-Regular.ttf";
    public final static String ROBOTO_BOLD = "fonts/Roboto-Bold.ttf";
    public final static String DIRECTORY_EVIDENCE = "/YoVerifico";
    public final static String TXT_YO_VERIFICO = "YoVerifico.txt";
    public final static String CONDITIONS_AND_TERMS = "Aplicación desarrollada by opi.la";
    public final static String DATA_FORMAT = "yyyyMMdd_HHmmss";
    public final static String DATA_FORMAT_PICTURE = "dd-MMM-yyyy - HH:mm:ss -";
    public final static String IMAGE_NAME_DEFOULT = "evidence -";
    public final static String JPG_EXTENSION = ".jpg";
    public final static String PNG_EXTENSION = ".png";
    public final static String ERROR_PARSER = "parser error States";
    public final static String ERROR_REGULAR_EXPRESION = "parser error html regex";
    public final static String ERROR_RETROFIT = "error connection";
    public final static String ERROR_REQUEST = "Error en peticion HTTP: ";

    public final static String DIALOG_TEXT = "dialog";
    public final static String UTF_8 = "UTF-8";
    public final static String MESSAGE = "message";

    public final static String TABLE = "CREATE TABLE EVIDENCIA (evidenceTitle TEXT , evidencePicture TEXT)";
    public final static String ON_UPGRADE_DROP = "DROP TABLE IF EXISTS EVIDENCIA";
    public final static String DATA_BASE_NAME = "EVIDENCIA_CURRENCIA";
    public final static String FIELD_EVIDENCE_NAME = "evidenceTitle";
    public final static String FIELD_EVIDENCE_PICTURE = "evidencePicture";
    public final static String TABLE_NAME = "EVIDENCIA";
    public final static String QUERY_PATH = "SELECT * FROM EVIDENCIA";

    public final static String TITLE_OCURRENCE = "title_ocurrence";
    public final static String DATA_OCURRENCE = "data_ocurrence";
    public final static String TIME_OCURRENCE = "time_ocurrence";
    public final static String DESCRIPTION_OCURRENCE = "descripcion_ocurrence";
    public final static String PLACE_OCURRENCE = "place_ocurrence";
    public final static String ID_OCURRENCE = "id_ocurrence";
    public final static String CONTACT_OCURRENCE = "contact_ocurrence";
    public final static String BUNDLE_OCURRENCE = "ocurrences_bundle";
    public final static String OCURRENCES_FRAGMENT = "ocurrences_fragment";
    public final static String WINDOINFO_FRAGMENT = "window_info_fragment";
    public final static String DETAIL_FRAGMENT = "detail_fragment";
    public final static String KEY_INTENT = "home";
    public final static String TUTORIAL = "tutorial";
    public final static String SHOWME_FROM_PREFERENCES_TUTORIAL = "showme_tutorial_preferences";
    public final static String SHOWME_TUTORIAL = "showme_tutorial_login";
    public final static String FRAGMENT_TUTORIAL = "tutorial";

}
